...
def compute_loss(model: nn.Module, x: torch.Tensor):
    ...
    left_boundary = torch.tensor([-1.0])
    right_boundary = torch.tensor([1.0])
    left_residual = model(left_boundary) - torch.tanh(left_boundary)
    right_residual = model(right_boundary) - torch.tanh(right_boundary)
    boundary_loss = torch.mean(left_residual ** 2 + right_residual ** 2)

    #calculating initial value loss for problem - include this for task 3
    x_initial = torch.tensor(0.0, requires_grad=True)
    #dirichlet_loss = ...
    #neumann_loss = ...
    #initial_loss = torch.mean(dirichlet_loss**2 + neumann_loss**2)
...
# The PINN is initialised
# 4000 steps of gradient descent are performed with ADAM on the loss function
